...
# Defensive column rename: the source CSV may arrive with either the
# raw warehouse column names (pickupdistance, traveldistance) or the
# Appendix 2 names used in the dashboard (pickup_distance,
# dropoff_distance). Map either form to the canonical names.
COLUMN_ALIASES = {
...
moves NPS and reorder rate, which are the metrics ops actually gets paid
on.</p>
</div>
""", unsafe_allow_html=True)
...
def days_from_payday(d):
    ...
    # Include previous month's last-day pay date
    if d.month == 1:
    ...
    with tab:
        ...
        if dim == "territory":
            insight = (f"Spread of just <strong>{spread:.1f} min</strong> "
                       ...
                       "Don't burn cycles re-routing. Focus on flow "
                       "and surface.")
            ...
            insight = (f"<strong>{spread:.1f} min</strong> spread across "
                       ...
                       "is closer to a Merchant Success project than a "
                       "tech build.")
        else:
...
with c1:
    ...
    # Always show the volume backdrop
