...
def days_from_payday(d):
    ...
    the preceding Friday when they fall on Sat or Sun.
...
with c2:
    # KPIs that compare paydays to different days
...
if abs(atd_gap) < 0.5:
    atd_call = ("Payday itself doesn't move ATD in any meaningful way "
                "in this 2-month window")
elif atd_gap > 0:
    atd_call = (f"Payday adjacent days run about {atd_gap:.1f} min "
                "slower than other days")
else:
    atd_call = (f"Payday adjacent days run about {abs(atd_gap):.1f} min "
...
for tab, dim, label in [
    ...
    with tab:
        ...
        # specific insight, with volume context
        if dim == "territory":
...
if cp.empty:
    ...
    route coaching, hot-zone hints, and re-tenuring.</p>
    </div>
    """, unsafe_allow_html=True)

# ============================================================
# Distance with markers
# ============================================================
st.markdown("<br><br>", unsafe_allow_html=True)
...
# distances into 1 km bins to draw a trend line .
